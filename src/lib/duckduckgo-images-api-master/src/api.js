...
async function image_search({ query, moderate, retries, iterations }) {
  ...
  if (!retries) {
    retries = max_retries
  } // default to max if none provided
  if (!iterations) {
    iterations = max_iter
  } // default to max if none provided
        ...
        try {
          ...
          try {
            ...
            Object.keys(params).forEach((key) => {
              urlStr = `${urlStr}${key}=${encodeURIComponent(params[key])}&`
            })
            u = new URL(urlStr);
          } catch (err) {
            console.error('cannot make url from ', reqUrl, 'and', params)
            throw err
          }
    ...
    while (itr < iterations) {
      ...
      while (true) {
        try {
          ...
          let response = await axios.get(new URL(reqUrl, {
            searchParam: params,
          }).href, {
            headers
          })
